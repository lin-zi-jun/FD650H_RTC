...
signed char mir3da_open_interrupt()
{
    ...
    res = mir3da_register_write(NSA_REG_INT_LATCH,0x0d);//Latch 50 ms

    res |= mir3da_register_write(NSA_REG_TAP_DURATION,0x45 );//Quiet 30ms & Shock 70ms &Dur 375ms

    res |= mir3da_register_write(NSA_REG_TAP_THRESHOLD,0x0D );//125mg * 0x0D
    
    res |= mir3da_register_write(NSA_REG_INTERRUPT_MAPPING1,0x10 );//Mappint the double-tap interrupt to int1

    res |= mir3da_register_write(NSA_REG_INTERRUPT_SETTINGS1,0x10);//Enable double-tap interrupt
